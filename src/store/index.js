...
const store = new Vuex.Store({
    ...
    state:{
        ...
        tabList: [
            ...
            {path: 'login', label: '登录', name: 'login'},
            {path: 'login2', label: '登录2', name: 'login2'},
        ],
        aboutactivePath:"about1",
        abouttabList:[
            {path: 'about1', label: '测试页面1', name: 'about1'}
        ]
    },
    // 更改vuex的store中状态的唯一方法 - 同步操作
    mutations: {
        aboutselectMenu(state,item){
            if(state.aboutactivePath == item.name){
                return
            }
            var result = state.abouttabList.some(tab => tab.name == item.name)
            if(!result){
                state.abouttabList.push(item)
            }
            state.aboutactivePath = item.name;
            
        },
        aboutcloseTab(state,tab){
